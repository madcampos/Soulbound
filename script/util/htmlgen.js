var HTMLGen = {
	...
	createInput : function (inputType, inputValue, selectedClass, inputClass, inputId, description) {
		...
		input.onfocus = function(event){
			event.target.previousElementSibling.className += (' ' + selectedClass);
		};
		
		input.onblur = function(event){
			event.target.previousElementSibling.className.substring(0,
				event.target.previousElementSibling.className.length - selectedClass.length);
		};
